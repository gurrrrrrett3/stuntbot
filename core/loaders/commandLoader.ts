...
export default class CommandLoader {
  ...
  async load(commands: CustomCommandBuilder[]) {
    ...
    if (process.env.COMMAND_MODE == "GUILD") {
      ...
      Logger.debug("CommandLoader", `Deploying ${commandsToDeploy.length} command${commandsToDeploy.length == 1 ? "" : "s"} to guild ${process.env.GUILD_ID as string}`);
      ...
      Logger.debug("CommandLoader", `Deploying ${commandsToDeploy.length} command${commandsToDeploy.length == 1 ? "" : "s"} globally`);
