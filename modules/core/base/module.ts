import Bot from "../bot";
import { BaseModuleType, CustomCommandBuilder, ModuleDefaultConfig } from "../loaders/loaderTypes";
import fs from "fs";
...
import PersistantStorage from "../managers/persistantStorage";

export default class Module<Config extends ModuleDefaultConfig = {}, PersistantStorageType = any>
  implements BaseModuleType
{
  ...
  public persistantStorage: PersistantStorage<PersistantStorageType>;

  constructor(bot: Bot, config?: Config) {
    this.client = bot.client;
    this.config = config || ({} as Config);
    this.persistantStorage = new PersistantStorage(this.name);
  ...
  public getConfig<Key extends keyof Config>(key: Key): Config[Key]["value"] {
